    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                mDefaultMtu = 23;
                Log.d(TAG, "MTU change failed. Status: " + status);
            }
        }
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                // 描述符写入成功
                if (descriptor.getCharacteristic().getUuid().toString().equals(SampleGattAttributes.TRANSMIT_READ_UUID)) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                        boolean requestMtuSuccess = gatt.requestMtu(mDefaultMtu);
                        Log.d(TAG, "Request MTU - " + (requestMtuSuccess ? "Request sent" : "Request failed"));
                    }
                }
            } else {
                // 描述符写入失败
                Log.i(TAG, "descriptor write fail.");
            }
        }
